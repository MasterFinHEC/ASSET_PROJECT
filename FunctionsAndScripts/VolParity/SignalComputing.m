function [Signal] = SignalComputing(Returns,firstData,LengthSignal,LengthMonth)
...
Signal = zeros(round((length(Returns)-LengthSignal)/LengthMonth,0),asset);
...
for i = 1:asset %For each asset 
    ...
    for j = LengthSignal+1:LengthMonth:length(Returns) %Loop going over each portfolio rebalancing (month)
        
        if j-LengthSignal >= firstData(i) %If the asset is available and has been for more than a year.
            
             Signal(position,i) = prod(Returns(j-LengthSignal:j-1,i))-1; %Computing the return of asset i between
                                                                %time j-252 and j.
