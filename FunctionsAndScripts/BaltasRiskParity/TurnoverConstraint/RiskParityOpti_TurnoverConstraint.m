...
disp('Optimisation is starting !')
    ...
    100000,'Display','none');
        ...
        % Optimizing the month's weights 
         WeightsOpti(position,index==1) = fmincon(fun,NetWeights(position,index==1),A,b,Aeq,beq,lb,ub,@(x)VolAndTurnover(x,Target,CovMat,TargetTurnover,MonReturn),options);
