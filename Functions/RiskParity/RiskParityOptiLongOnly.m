function [WeightsOpti,MCRriskParity,Exitflag] = RiskParityOptiLongOnly(Weights,Returns,Target,LengthSignal,LengthVol,LengthMonth)
...
disp('Optimisation is starting !')
    ...
    for i = LengthSignal+1:LengthMonth:length(Returns) 
        ...
        fun = @(x) -sum(log(x)); %Function going over the available assets (index == 1)
