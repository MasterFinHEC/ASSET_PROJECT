function [NetWeights,weights,MCR] = Allocation_VolParity(Returns,firstData,LengthSignal,LengthVol,LengthMonth,Signal)
...
MCR = zeros(round((length(Returns)-LengthSignal)/LengthMonth,0),asset); %Matrice to store the Weights
...
%Computing the weights for each month
    for i = LengthSignal+1:LengthMonth:length(Returns) %Start of the strategy, every month, until the end
      ...
      %Computing the Gross weights for the available assets
        ...
        %Computing the contribution to risk
        CovMat = cov(Returns(i-LengthVol+1:i,available ==1));
        MCR(position,available==1) = (weights(position,available==1)'.*(CovMat*weights(position,available==1)')/(weights(position,available==1)*CovMat*weights(position,available==1)'))'.*100;
   position = position + 1; %Increasing the position to go over the next month
   
    end 
    
    % Computing the Net Weights
